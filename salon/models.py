...
class Salon(BaseModel):
    ...
    ), on_delete=models.CASCADE, verbose_name='کاربر', related_name='salon')
...
class Review(BaseModel):
    user = models.ForeignKey(
        get_user_model(), on_delete=models.SET_NULL, null=True, verbose_name='کاربر')
...
class CourtSize(BaseModel):
    ...
    size = models.CharField(max_length=100, choices=SIZE_CHOICES,
                            default=LARGE, verbose_name='اندازه زمین ورزشی', unique=True)
...
class Court(BaseModel):
    ...
    size = models.ForeignKey(
        CourtSize, on_delete=models.PROTECT, verbose_name='اندازه')
...
class Reservation(BaseModel):
    court = models.ForeignKey("Court", on_delete=models.PROTECT,
                              related_name="reservations", verbose_name='نوع سالن ورزشی')
    user = models.ForeignKey(
        get_user_model(), on_delete=models.CASCADE, verbose_name='کاربر')
    available_time = models.ManyToManyField(
        AvailableTime, blank=True, verbose_name='زمان های در دسترس')
    date = models.DateField(blank=True, null=True, verbose_name='تاریخ')
    is_paid = models.BooleanField(default=False, verbose_name='پرداخت شده')
    additional_notes = models.TextField(
        blank=True, verbose_name='توضیحات اضافه')
    booking_reference = models.ForeignKey(
        "BookingReference", on_delete=models.CASCADE, blank=True, verbose_name='شماره رزرو')
    is_cancelled = models.BooleanField(default=False, verbose_name='کنسل شده')
    total_amount = models.DecimalField(
        max_digits=15, decimal_places=0, default=200, verbose_name='کل مبلغ')
...
class SiteReview(BaseModel):
    user = models.ForeignKey(
        get_user_model(), on_delete=models.CASCADE, verbose_name='کاربر', blank=True)
